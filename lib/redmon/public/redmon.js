var Redmon = (function() {
  ...
  var memoryWidget = (function() {
    ...
    function render(data) {
      ...
      new Rickshaw.Graph.Axis.Y({
        element: $('#memory-container .y-axis').get(0),
        graph: graph,
        orientation: 'left',
        tickFormat: base1024
      });
  ...
  var keyspaceWidget = (function() {
    var graph
      , dataset = {
        hits:   {name: 'Hits',   color: 'steelblue', data: []},
        misses: {name: 'Misses', color: 'red',  data: []},
        load: function(data) {
          var self = this;
        ...
        append: function(data) {
          if (this.hits.data.length >= 100) this.shift();
    ...
    function render(data) {
      dataset.load(data);
      graph = new Rickshaw.Graph({
        element: $('#keyspace-chart').get(0),
        renderer: 'bar',
        width: 820,
        height: 215,
        series: [dataset.misses, dataset.hits]
      });

      new Rickshaw.Graph.Axis.Y({
        element: $('#keyspace-container .y-axis').get(0),
        graph: graph,
        orientation: 'left',
      ...
      new Rickshaw.Graph.HoverDetail({
        ...
        xFormatter: formatDate,
        formatter: function(series, x, y) {
          return series.name + ': ' + formatNumber(y)
        }
      });

      var legend = new Rickshaw.Graph.Legend({
        graph: graph,
        element: $('#keyspace-legend-container').get(0),
      });
